...
// Electric purple/pink neon on dark violet — high contrast, buzzing energy
function seeded(s: number) {
  return () => { s = (s * 16807) % 2147483647; return s / 2147483647; };
}

export const NeonPurple: React.FC<VariantProps> = ({ width, height }) => {
  ...
  useEffect(() => {
    ...
    const grad = ctx.createRadialGradient(width / 2, height * 0.6, 0, width / 2, height / 2, width * 0.8);
    grad.addColorStop(0, "#1a0830");
    grad.addColorStop(0.6, "#0e0420");
    ...
    for (const [a, b] of data.edges) {
      ...
      ctx.strokeStyle = "rgba(200, 80, 255, 0.22)";
    ...
    for (let i = 0; i < visible; i++) {
      ...
      const pulse = 0.8 + Math.sin(frame * 0.07 + i * 2) * 0.2;

      const g = ctx.createRadialGradient(n.x, n.y, 0, n.x, n.y, n.r * 6);
      g.addColorStop(0, `rgba(220, 100, 255, ${alpha * pulse * 0.3})`);
      g.addColorStop(1, "rgba(180, 50, 255, 0)");
      ctx.fillStyle = g;
      ctx.beginPath();
      ctx.arc(n.x, n.y, n.r * 6, 0, Math.PI * 2);
      ctx.fill();

      ctx.fillStyle = `rgba(240, 200, 255, ${alpha * pulse * 0.95})`;
      ctx.beginPath();
