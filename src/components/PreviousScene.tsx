...
export function PreviousScene({ scene }: { scene: Scene | null }) {
  ...
  return (
    <div className="h-full flex flex-col rounded-lg border border-white/5 bg-[#0e0e18] p-3">
      ...
      {/* Placeholder for Remotion Player — will show the picked variant */}
      <div className="flex-1 rounded bg-[#1a1a2e] flex items-center justify-center mb-2 min-h-[120px]">
        <div className="text-center">
          <span className="text-white/20 text-xs block">{scene.compositionId}</span>
          <span className="text-white/10 text-[10px] block mt-1">
            {scene.status === "final" ? "Picked variant " + scene.pickedVariant : "Not picked"}
          </span>
        </div>
      </div>
